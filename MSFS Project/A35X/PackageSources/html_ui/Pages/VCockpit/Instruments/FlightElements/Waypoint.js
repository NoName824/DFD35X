...
class WayPointInfo {
    ...
    SetFromIFacilityWaypoint(data) {
        ...
        this.city =
            separatedCity.length > 1
                ? Utils.Translate(separatedCity[0]) + ', ' + Utils.Translate(separatedCity[1])
                : Utils.Translate(data.city);
        this.lat = data.lat;
        this.long = data.lon;
    }
    ...
    SetFromIFacilityAirport(data, loadApproachesData = true) {
        ...
        if (data.frequencies) {
            for (let i = 0; i < data.frequencies.length; i++) {
                this.frequencies.push(
                    new Frequency(data.frequencies[i].name, data.frequencies[i].freqMHz, data.frequencies[i].freqBCD16)
                );
            }
        }
        this.runways = [];
                ...
                for (let i = 0; i < approachData.transitions.length; i++) {
                    ...
                    for (let j = 0; j < approachData.transitions[i].legs.length; j++) {
                        ...
                        if (loadApproachesData) {
                            this.instrument.facilityLoader
                                .getFacility(approachData.transitions[i].legs[j].fixIcao)
                                .then(
                                    function (_legs, _index, _waypoint) {
                                        if (_waypoint) {
        ...
        for (let i = 0; i < this.departures.length; i++) {
            for (let j = 0; j < this.departures[i].runwayTransitions.length; j++) {
                this.departures[i].runwayTransitions[j].name =
                    'RW' + this.departures[i].runwayTransitions[j].runwayNumber.toString();
            ...
            for (let j = 0; j < this.departures[i].enRouteTransitions.length; j++) {
                ...
                this.departures[i].enRouteTransitions[j].name = this.departures[i].enRouteTransitions[j].legs[
                    legsCount - 1
                ].fixIcao.substr(7, 5);
            }
        }
        ...
        for (let i = 0; i < this.arrivals.length; i++) {
            for (let j = 0; j < this.arrivals[i].runwayTransitions.length; j++) {
                this.arrivals[i].runwayTransitions[j].name =
                    'RW' + this.arrivals[i].runwayTransitions[j].runwayNumber.toString();
            ...
            for (let j = 0; j < this.arrivals[i].enRouteTransitions.length; j++) {
                this.arrivals[i].enRouteTransitions[j].name = this.arrivals[i].enRouteTransitions[
                    j
                ].legs[0].fixIcao.substr(7, 5);
            }
        }
    }
...
class IntersectionInfo extends WayPointInfo {
    ...
    UpdateInfos(_CallBack = null, loadFacilitiesTransitively = true) {
        ...
        this.instrument.facilityLoader.getIntersectionDataCB(this.icao, (data) => {
            this.SetFromIFacilityIntersection(
                data,
                () => {
                    this.loaded = true;
                    if (_CallBack) {
    ...
    SetFromIFacilityIntersection(data, callback = EmptyCallback.Void, loadAirways = true) {
        ...
        this.nearestVORIdent = data.nearestVorICAO
            ? data.nearestVorICAO.substring(7, 12).replace(new RegExp(' ', 'g'), '')
            : '';
