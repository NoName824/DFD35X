...
export class ManagedFlightPlan {
    ...
    public get checksum(): number {
        ...
        for (let i = 0; i < waypoints.length; i++) {
            ...
            checksum +=
                waypoints[i].legAltitude1 +
                waypoints[i].legAltitude2 +
                waypoints[i].legAltitudeDescription +
                waypoints[i].speedConstraint;
        }
        return checksum;
    }
    ...
    public addWaypoint(waypoint: WayPoint, index?: number | undefined, segmentType?: SegmentType): void {
        const mappedWaypoint =
            waypoint instanceof WayPoint ? waypoint : RawDataMapper.toWaypoint(waypoint, this._parentInstrument);
        if (mappedWaypoint.type === 'A' && index === 0) {
            ...
            let segment =
                segmentType !== undefined ? this.getSegment(segmentType) : this.findSegmentByWaypointIndex(index);
            ...
            if (segment) {
                ...
                if (this.activeWaypointIndex === 0 && this.length > 1) {
                    ...
                    this.activeWaypointIndex === 1 &&
                    waypoint.isRunway &&
                    segment.type === SegmentType.Departure
                ) {
    ...
    public reflowDistances(): void {
        ...
        for (let i = 0; i < waypoints.length; i++) {
            if (i > 0) {
                ...
                const trueCourseToWaypoint = Avionics.Utils.computeGreatCircleHeading(
                    prevWaypoint.infos.coordinates,
                    referenceWaypoint.infos.coordinates
                );
                referenceWaypoint.bearingInFP =
                    trueCourseToWaypoint -
                    GeoMath.getMagvar(prevWaypoint.infos.coordinates.lat, prevWaypoint.infos.coordinates.long);
                referenceWaypoint.bearingInFP =
                    referenceWaypoint.bearingInFP < 0
                        ? 360 + referenceWaypoint.bearingInFP
                        : referenceWaypoint.bearingInFP;
                referenceWaypoint.distanceInFP = Avionics.Utils.computeGreatCircleDistance(
                    prevWaypoint.infos.coordinates,
                    referenceWaypoint.infos.coordinates
                );
    ...
    public copy(): ManagedFlightPlan {
        ...
        for (let i = 0; i < this._segments.length; i++) {
            ...
            newFlightPlan._segments[i].waypoints = [
                ...seg.waypoints.map((w) => Object.assign(new WayPoint(w.instrument), w))
            ];
        }
        ...
        newFlightPlan.directTo.interceptPoints =
            this.directTo.interceptPoints !== undefined ? [...this.directTo.interceptPoints] : undefined;
    ...
    public calculateDirectIntercept(waypoint: WayPoint): WayPoint[] {
        ...
        const coords = Avionics.Utils.bearingDistanceToCoordinates(
            planeHeading,
            Math.min(interceptDistance, 1.0),
            lat,
            long
        );
    ...
    */
        ...
        if (legs.length > 0 || selectedOriginRunwayIndex !== -1 || (departureIndex !== -1 && runwayIndex !== -1)) {
            ...
            if (selectedOriginRunwayIndex !== -1) {
                ...
                runway = this.getRunway(
                    airportInfo.oneWayRunways,
                    airportInfo.departures[departureIndex].runwayTransitions[runwayIndex].name
                );
            }
            ...
            if (runway) {
                ...
                const runwayWaypoint = procedure.buildWaypoint(
                    `RW${selectedRunwayOutput}`,
    ...
    public async buildArrival(): Promise<void> {
        ...
        if (legs.length > 0) {
            ...
            const procedure = new LegsProcedure(
                legs,
                this.getWaypoint(segment.offset - 1),
                this.getWaypoint(segment.offset - 2),
    ...
    public async buildApproach(): Promise<void> {
        ...
        if (legs.length > 0 || approachIndex !== -1 || destinationRunwayIndex !== -1) {
            ...
            const procedure = new LegsProcedure(
                legs,
                this.getWaypoint(startIndex - 1),
                this.getWaypoint(startIndex - 2),
                this._parentInstrument
            );
            ...
            if (approachIndex !== -1) {
                runway = this.getRunway(
                    destinationInfo.oneWayRunways,
                    destinationInfo.approaches[approachIndex].runway
                );
            ...
            if (runway) {
                ...
                if (approachIndex === -1 && destinationRunwayIndex !== -1 && destinationRunwayExtension !== -1) {
                    const runwayExtensionWaypoint = procedure.buildWaypoint(
                        `RX${selectedRunwayOutput}`,
                        Avionics.Utils.bearingDistanceToCoordinates(
                            runway.direction + 180,
                            destinationRunwayExtension,
                            runway.beginningCoordinates.lat,
                            runway.beginningCoordinates.long
                        )
                    );
                ...
                if (approachIndex !== -1) {
                    ...
                    const missedProcedure = new LegsProcedure(
                        destinationInfo.approaches[approachIndex].missedLegs,
                        this.getWaypoint(missedStartIndex - 1),
                        this.getWaypoint(missedStartIndex - 2),
                        this._parentInstrument
                    );
    ...
    public truncateSegment(type: SegmentType): { startIndex: number; segment: FlightPlanSegment } {
        ...
        const startIndex =
            this.findSegmentByWaypointIndex(this.activeWaypointIndex) === segment
                ? this.activeWaypointIndex + 1
                : segment.offset;
        ...
        if (segment.waypoints.length === 0) {
            ...
            segment.waypoints[
                Math.min(Math.max(startIndex - 1 - segment.offset, 0), segment.waypoints.length - 1)
            ].endsInDiscontinuity = true;
        }
    ...
    public getRunway(runways: OneWayRunway[], runwayName: string): OneWayRunway {
        if (runways.length > 0) {
            ...
            if (runwayLetter === ' ' || runwayLetter === 'C') {
                ...
                runwayIndex = runways.findIndex(
                    (r) => r.designation === runwayDirection || r.designation === `${runwayDirection}C`
                );
            } else {
