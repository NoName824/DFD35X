...
export class FlightPlanManager {
    ...
    public setActiveWaypointIndex(
        ...
        if (index >= 0 && index < currentFlightPlan.length) {
            ...
            if (
                currentFlightPlan.directTo.isActive &&
                currentFlightPlan.directTo.waypointIsInFlightPlan &&
        ...
        if (destination) {
            ...
            if (approach) {
                ...
                if (approach.transitions.length > 0) {
                    approachTransition =
                        approach.transitions[currentFlightPlan.procedureDetails.approachTransitionIndex];
                }
    ...
    public getDepartureRunway(): OneWayRunway {
        const currentFlightPlan = this._flightPlans[this._currentFlightPlanIndex];
        if (
            currentFlightPlan.hasOrigin &&
            currentFlightPlan.procedureDetails.departureRunwayIndex !== -1 &&
            currentFlightPlan.procedureDetails.departureIndex !== -1
        ) {
            const depRunway = (currentFlightPlan.originAirfield.infos as AirportInfo).departures[
                currentFlightPlan.procedureDetails.departureIndex
            ].runwayTransitions[currentFlightPlan.procedureDetails.departureRunwayIndex].name.replace('RW', '');

            const runway = (currentFlightPlan.originAirfield.infos as AirportInfo).oneWayRunways.find((r) => {
            ...
            return (currentFlightPlan.originAirfield.infos as AirportInfo).oneWayRunways[
                currentFlightPlan.procedureDetails.originRunwayIndex
            ];
        }
     ...
     * @param callback A callback to call when the operation completes.
     */
    public async setDestinationRunwayIndex(
        index: number,
        runwayExtension: number = -1,
        callback: () => void = () => {}
    ): Promise<void> {
        const currentFlightPlan = this._flightPlans[this._currentFlightPlanIndex];
    ...
    public isActiveApproach(forceSimVarCall = false): boolean {
        const currentFlightPlan = this._flightPlans[this._currentFlightPlanIndex];
        return (
            currentFlightPlan.approach.waypoints.length > 0 &&
    ...
    public getApproach(): any {
        ...
        if (currentFlightPlan.hasDestination && currentFlightPlan.procedureDetails.approachIndex !== -1) {
            return (currentFlightPlan.destinationAirfield.infos as AirportInfo).approaches[
                currentFlightPlan.procedureDetails.approachIndex
            ];
        }
    ...
    public getApproachNavFrequency(): number {
        ...
        if (approach && approach.name.includes('ILS')) {
            ...
            const frequency = aptInfo.namedFrequencies.find(
                (f) => f.name.replace('RW0', '').replace('RW', '').indexOf(approachRunway) !== -1
            );
    ...
    public getApproachRunway(): OneWayRunway {
        ...
        if (currentFlightPlan.hasDestination && currentFlightPlan.procedureDetails.approachIndex !== -1) {
            ...
            const approachRunwayName = (destination.infos as AirportInfo).approaches[
                currentFlightPlan.procedureDetails.approachIndex
            ].runway;

            const runway = currentFlightPlan.getRunway(
                (destination.infos as AirportInfo).oneWayRunways,
                approachRunwayName
            );
            return runway;
        }
    ...
    public getCoordinatesAtNMFromDestinationAlongFlightPlan(distance: number): LatLongAlt {
        ...
        if (destination) {
            ...
            for (let i = 0; i < allWaypoints.length - 1; i++) {
                ...
                if (
                    prev.cumulativeDistanceInFP < fromStartDistance &&
                    next.cumulativeDistanceInFP > fromStartDistance
                ) {
                    break;
                }
            }
