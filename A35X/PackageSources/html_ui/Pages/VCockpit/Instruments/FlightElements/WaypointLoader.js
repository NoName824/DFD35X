class NearestAirspacesLoader {
    ...
    update() {
        if (
            !this._updating &&
            (this._lastlla == undefined ||
                Math.abs(this._lastlla.lat - this.lla.lat) > 0.25 ||
                Math.abs(this._lastlla.long - this.lla.long) > 0.25)
        ) {
            this._updating = true;
...
class FacilityLoader {
    ...
    addFacility(_data, type) {
        ...
        let facilityIndex = this.loadedFacilities.findIndex(
            (f) =>
                f.icao === _data.icao &&
                ((f.routes === undefined && _data.routes === undefined) ||
                    (f.routes !== undefined && _data.routes !== undefined))
        );
    ...
    getFacilityRaw(icao, timeout = 1500) {
        ...
        switch (type) {
            ...
            case 'V':
                return Promise.all([
                    queueRawLoad('LOAD_VOR', icao, 'V'),
                    queueRawLoad('LOAD_INTERSECTION', icao, 'W')
                ]).then((facilities) => {
                    return Object.assign(facilities[0], facilities[1]);
                });
            case 'N':
                return Promise.all([
                    queueRawLoad('LOAD_NDB', icao, 'N'),
                    queueRawLoad('LOAD_INTERSECTION', icao, 'W')
    ...
    getAirportDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            let airport = this.loadedFacilities.find((f) => {
        ...
        return new Promise((resolve) => {
            ...
            let loadedAirportsCallback = () => {
                ...
                if (icaos.length === 0) {
                    ...
                    if (attempts > 10) {
                        console.warn(
                            'getAirportsDatas not found for ' + icaos.length + ' icaos, expect the unexpected.'
                        );
                        resolve(datas);
                    } else {
    ...
    getNdbDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            let ndb = this.loadedFacilities.find((f) => {
                return f.icaoTrimed === icao && f.routes === undefined;
            });
            if (ndb) {
                return callback(ndb);
    ...
    getVorDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            let vor = this.loadedFacilities.find((f) => {
                return f.icaoTrimed === icao && f.routes === undefined;
            });
            if (vor) {
                return callback(vor);
        ...
        if (intersectionInfo.routes) {
            for (let i = 0; i < intersectionInfo.routes.length; i++) {
                if (name === undefined || name === intersectionInfo.routes[i].name) {
                    ...
                    if (!airwayData) {
                        airwayData = await this.getAirwayData(
                            intersectionInfo,
                            intersectionInfo.routes[i].name,
                            maxLength
                        );
    ...
    async getAirwayData(intersectionInfo, name = '', maxLength = 100) {
        ...
        let route = intersectionInfo.routes.find((r) => {
            return r.name === name;
        });
        if (route) {
            let airway = {
    ...
    async GetAirportNamedFrequenciesByIdent(airportIdent) {
        ...
        return new Promise((resolve) => {
            SimVar.SetSimVarValue(
                'C:fs9gps:IcaoSearchStartCursor',
                'string',
                'A',
                ...
                this.instrument.requestCall(() => {
                    SimVar.SetSimVarValue(
                        ...
                        SimVar.SetSimVarValue(
                            ...
                            let airportIcao = SimVar.GetSimVarValue(
    ...
    async GetAirportNamedFrequencies(airportIcao) {
        ...
        return new Promise((resolve) => {
            SimVar.SetSimVarValue(
                ...
                let frequencyCount = SimVar.GetSimVarValue(
                    'C:fs9gps:WaypointAirportFrequenciesNumber',
                    'number',
                    this.instrument.instrumentIdentifier + '-loader'
                );
                let attempts = 0;
                ...
                while (frequencyCount === 0 && attempts < 10) {
                    ...
                    frequencyCount = SimVar.GetSimVarValue(
                        'C:fs9gps:WaypointAirportFrequenciesNumber',
                        'number',
                        this.instrument.instrumentIdentifier + '-loader'
                    );
                ...
                let getFrequency = async (index) => {
                    return new Promise((resolve) => {
                        SimVar.SetSimVarValue(
                            'C:fs9gps:WaypointAirportCurrentFrequency',
                            'number',
                            index,
                            this.instrument.instrumentIdentifier + '-loader'
                        ).then(() => {
                            let frequencyName = SimVar.GetSimVarValue(
                                'C:fs9gps:WaypointAirportFrequencyName',
                                'string',
                                this.instrument.instrumentIdentifier + '-loader'
                            );
                            let frequencyValue = SimVar.GetSimVarValue(
                                'C:fs9gps:WaypointAirportFrequencyValue',
                                'number',
                                this.instrument.instrumentIdentifier + '-loader'
                            );
                            resolve({
...
class WaypointLoader {
    ...
    update() {
        ...
        if (!this._isLoadingItems) {
            ...
            if (
                t - this._lastSearchOriginSyncDate > this.deprecationDelay ||
                deltaLat > this.currentMapAngularHeight * 0.5 ||
                deltaLong > this.currentMapAngularWidth * 0.5
            ) {
                this._locked = true;
                SimVar.SetSimVarValue(
                    'C:fs9gps:' + this.SET_ORIGIN_LATITUDE,
                    'degree latitude',
                    this._searchOrigin.lat,
                    this.instrument.instrumentIdentifier + '-loader'
                ).then(() => {
                    SimVar.SetSimVarValue(
                        'C:fs9gps:' + this.SET_ORIGIN_LONGITUDE,
                        'degree longitude',
                        this._searchOrigin.long,
                        this.instrument.instrumentIdentifier + '-loader'
                    ).then(() => {
            ...
            if (this._searchRangeNeedUpdate || t - this._lastSearchRangeSyncDate > this.deprecationDelay) {
                this._locked = true;
                SimVar.SetSimVarValue(
                    'C:fs9gps:' + this.SET_SEARCH_RANGE,
                    'nautical miles',
                    this.searchRange,
                    this.instrument.instrumentIdentifier + '-loader'
                ).then(() => {
                    let trueSearchRange = SimVar.GetSimVarValue(
                        'C:fs9gps:' + this.GET_SEARCH_RANGE,
                        'nautical miles',
                        this.instrument.instrumentIdentifier + '-loader'
                    );
            ...
            if (
                this._maxItemsSearchCountNeedUpdate ||
                t - this._lastMaxItemsSearchCountSyncDate > this.deprecationDelay
            ) {
                this._locked = true;
                SimVar.SetSimVarValue(
                    'C:fs9gps:' + this.SET_MAX_ITEMS,
                    'number',
                    this.maxItemsSearchCount,
                    this.instrument.instrumentIdentifier + '-loader'
                ).then(() => {
                    let trueMaxItemsSearchCount = SimVar.GetSimVarValue(
                        'C:fs9gps:' + this.GET_MAX_ITEMS,
                        'number',
                        this.instrument.instrumentIdentifier + '-loader'
                    );
            ...
            if (this._itemsCountNeedUpdate || t - this._lastItemCountUpdateDate > this.deprecationDelay) {
                this._itemsCount = SimVar.GetSimVarValue(
                    'C:fs9gps:' + this.GET_ITEMS_COUNT,
                    'number',
                    this.instrument.instrumentIdentifier + '-loader'
                );
            ...
            if (this._itemsNeedUpdate) {
                ...
                if (this.createWaypointsCallback) {
                    if (!this.batch) {
                        ...
                        this.batch = new SimVar.SimVarBatch(
                            'C:fs9gps:' + this.GET_ITEMS_COUNT,
                            'C:fs9gps:' + this.SET_ITEM_INDEX
                        );
                    ...
                    SimVar.SetSimVarValue(
                        ...
                        if (this.GET_ITEM_ICAO && this.createWaypointCallback) {
                            let icao = SimVar.GetSimVarValue(
                                'C:fs9gps:' + this.GET_ITEM_ICAO,
                                'string',
                                this.instrument.instrumentIdentifier + '-loader'
                            );
                            ...
                            let ident = SimVar.GetSimVarValue(
                                'C:fs9gps:' + this.GET_ITEM_IDENT,
                                'string',
                                this.instrument.instrumentIdentifier + '-loader'
                            );
...
class NDBLoader extends WaypointLoader {
    constructor(_instrument) {
        ...
        this.createCallback = async (ident) => {
            ...
            ndb.ident = ident;
            ndb.icao = SimVar.GetSimVarValue(
                'C:fs9gps:NearestNdbCurrentICAO',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            ndb.coordinates = new LatLongAlt(
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestNdbCurrentNdbLatitude',
                    'degree latitude',
                    this.instrument.instrumentIdentifier + '-loader'
                ),
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestNdbCurrentNdbLongitude',
                    'degree longitude',
                    this.instrument.instrumentIdentifier + '-loader'
                )
            );
            ndb.frequencyMHz = SimVar.GetSimVarValue(
                'C:fs9gps:NearestNdbCurrentFrequency',
                'Megahertz',
                this.instrument.instrumentIdentifier + '-loader'
            );
            ndb.ndbType = SimVar.GetSimVarValue(
                'C:fs9gps:NearestNdbCurrentType',
                'number',
                this.instrument.instrumentIdentifier + '-loader'
            );
            return ndb;
...
class VORLoader extends WaypointLoader {
    constructor(_instrument) {
        ...
        this.createCallback = async (ident) => {
            ...
            vor.ident = ident;
            vor.icao = SimVar.GetSimVarValue(
                'C:fs9gps:NearestVorCurrentICAO',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            vor.coordinates = new LatLongAlt(
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestVorCurrentVorLatitude',
                    'degree latitude',
                    this.instrument.instrumentIdentifier + '-loader'
                ),
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestVorCurrentVorLongitude',
                    'degree longitude',
                    this.instrument.instrumentIdentifier + '-loader'
                )
            );
            vor.frequencyMHz = SimVar.GetSimVarValue(
                'C:fs9gps:NearestVorCurrentFrequency',
                'Megahertz',
                this.instrument.instrumentIdentifier + '-loader'
            );
            vor.frequencyBCD16 = SimVar.GetSimVarValue(
                'C:fs9gps:NearestVorCurrentFrequency',
                'Frequency BCD16',
                this.instrument.instrumentIdentifier + '-loader'
            );
            vor.vorType = SimVar.GetSimVarValue(
                'C:fs9gps:NearestVorCurrentType',
                'number',
                this.instrument.instrumentIdentifier + '-loader'
            );
            return vor;
...
class IntersectionLoader extends WaypointLoader {
    constructor(_instrument, showNamedOnly = false, showTermWpts = false, loadername = 'IntersectionLoader') {
        ...
        this.createCallback = async (ident) => {
            ...
            intersection.icao = SimVar.GetSimVarValue(
                'C:fs9gps:NearestIntersectionCurrentICAO',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            intersection.coordinates = new LatLongAlt(
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestIntersectionCurrentIntersectionLatitude',
                    'degree latitude',
                    this.instrument.instrumentIdentifier + '-loader'
                ),
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestIntersectionCurrentIntersectionLongitude',
                    'degree longitude',
                    this.instrument.instrumentIdentifier + '-loader'
                )
            );
            let routesCount = SimVar.GetSimVarValue(
                'C:fs9gps:NearestIntersectionCurrentRouteNumber',
                'number',
                this.instrument.instrumentIdentifier + '-loader'
            );
            if (routesCount > 0) {
            ...
            for (let i = 0; i < routesCount; i++) {
                SimVar.SetSimVarValue(
                    'C:fs9gps:NearestIntersectionCurrentCurrentRoute',
                    'number',
                    i,
                    this.instrument.instrumentIdentifier + '-loader'
                ).then(() => {
                    let routeName = SimVar.GetSimVarValue(
                        'C:fs9gps:NearestIntersectionCurrentRouteName',
                        'string',
                        this.instrument.instrumentIdentifier + '-loader'
                    );
                    let routePrevIcao = SimVar.GetSimVarValue(
                        'C:fs9gps:NearestIntersectionCurrentRoutePrevIcao',
                        'string',
                        this.instrument.instrumentIdentifier + '-loader'
                    );
                    let routeNextIcao = SimVar.GetSimVarValue(
                        'C:fs9gps:NearestIntersectionCurrentRouteNextIcao',
                        'string',
                        this.instrument.instrumentIdentifier + '-loader'
                    );
    ...
    async setCustomFs9Filter() {
        return new Promise((resolve) => {
            if (this._showNamedOnly === true) {
                SimVar.SetSimVarValue(
                    'C:fs9gps:' + this.SET_INTERS_FILTER,
                    'number',
                    2,
                    this.instrument.instrumentIdentifier + '-loader'
                )
...
class AirportLoader extends WaypointLoader {
    constructor(_instrument, loadFacility = true) {
        ...
        this.createCallback = async (ident) => {
            ...
            airport.ident = ident;
            airport.icao = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentICAO',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.coordinates = new LatLongAlt(
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestAirportCurrentAirportLatitude',
                    'number',
                    this.instrument.instrumentIdentifier + '-loader'
                ),
                SimVar.GetSimVarValue(
                    'C:fs9gps:NearestAirportCurrentAirportLongitude',
                    'number',
                    this.instrument.instrumentIdentifier + '-loader'
                )
            );
            airport.bestApproach = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentBestApproach',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.frequencyName = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentComFrequencyName',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.frequencyMHz = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentComFrequencyValue',
                'MHz',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.frequencyBCD16 = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentComFrequencyValue',
                'Frequency BCD16',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.longestRunwayLength = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentLongestRunwayLength',
                'feet',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.longestRunwayDirection = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentLongestAirportDirection',
                'degree',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.airportClass = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentAirportKind',
                'number',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.fuel1 = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentFuel1',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.fuel2 = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentFuel2',
                'string',
                this.instrument.instrumentIdentifier + '-loader'
            );
            airport.towered = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportCurrentTowered',
                'Boolean',
                this.instrument.instrumentIdentifier + '-loader'
            );
            let departuresCount = SimVar.GetSimVarValue(
                'C:fs9gps:NearestAirportDeparturesNumber',
                'number',
                this.instrument.instrumentIdentifier + '-loader'
            );
            let getDeparture = async (lineIndex) => {
                return new Promise((resolve) => {
                    SimVar.SetSimVarValue(
                        'C:fs9gps:NearestAirportCurrentDeparture',
                        'number',
                        lineIndex,
                        this.instrument.instrumentIdentifier + '-loader'
                    ).then(async () => {
                        let departureWaypointsCount = SimVar.GetSimVarValue(
                            'C:fs9gps:NearestAirportDepartureWaypointsNumber',
                            'number',
                            this.instrument.instrumentIdentifier + '-loader'
                        );
